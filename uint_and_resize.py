...
for files in os.listdir(in_dir):
   ...
   img8 = cv2.normalize(oimg, None, 0, 255, cv2.NORM_MINMAX, dtype=cv2.CV_8U)  
   small = cv2.resize(img8, (2000,2000), fx=0.5, fy=0.5, interpolation = cv2.INTER_AREA)
  ...
  # cv2.imwrite(os.path.join(outdir , 'small.tiff'), small)
